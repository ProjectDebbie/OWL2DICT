...
# get filename
path, file = os.path.split("/location/of/DEB_ontology.owl")
...
with open("DEB_ONTOLOGY.lst", 'w') as f:
    ...
    #clean alternative labels
    altLabs = {k: v.replace("'", "") for k, v in altLabs.items()}
    ...
    merged.update({key: [value1, value2, value3, value4, value5] for key, value1, value2, value3, value4, value5 in
                   zip(dict_superclasses.keys(), dict_superclasses.values(), ontology_path.values(), synonyms.values(),
    ...
    for k, v in merged.items():
        ...
        if k.isupper() == True:
            ...
            clean_altLab = re.sub(r"([A-Z])", r" \1", altLab).split()
            k = ' '.join(clean_k)
            k = k.replace("3 D", "3D")
            k = k.replace("- ", "-")
            k = k.lower()
            ...
            altLab = ' '.join(clean_altLab)
            altLab = altLab.replace("3 D", "3D")
            altLab = altLab.replace("- ", "-")
            altLab = altLab.lower()
            property = ' '.join(clean_property)
            property = property.replace("3 D", "3D")
            property = property.replace("- ", "-")
            property = property.lower()
        f.write(k + '\t' + 'LABEL=' + str(v[0]) + '\t' + 'PATH=' + str(v[1]) + '\t' + 'SYNONYM=' + str(
            synonym) + '\t' + 'PROPERTY=' + str(property) + '\t' + 'ALTLABEL=' + str(altLab) +  '\n')
